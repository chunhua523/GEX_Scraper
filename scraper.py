    ...
    async def run_scraping_job(self, tickers: list, models: list, cme_tickers: list, cme_models: list, download_folder: str, parallel_mode: bool = False):
        ...
        if tickers and models:
            for model in models:
                ...
                coro = self.process_model_queue(context, model, tickers, download_folder, tv_codes_std, target_url=BASE_URL, subfolder_prefix="")
                if parallel_mode:
                    tasks.append(coro)
                else:
                    await coro
        ...
        if cme_tickers and cme_models:
            CME_URL = "https://www.lietaresearch.com/platform/cme"
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if model == "TV Code":
                    ...
                    await page.wait_for_selector("text=Put Wall", state="visible", timeout=15000) 
